...
export function RifasSection({ negocioId, inicial }: { negocioId: string; inicial: Rifa[] }) {
  ...
  async function crear(e: React.FormEvent) {
    ...
    const { data } = await supabase
      .from('rifa')
      .insert({ negocio_id: negocioId, nombre, premio: premio || null, cierra_en: cierra || null })
      .select('id, nombre, premio, cierra_en, estado')
      .single();
    if (data) { setLista([data as Rifa, ...lista]); setNombre(''); setPremio(''); setCierra(''); }
  }
  ...
  return (
    <Card>
      <h3 className="mb-3 font-medium text-gray-900">Rifas</h3>
      <form onSubmit={crear} className="grid grid-cols-1 gap-3 md:grid-cols-4 md:items-end">
        ...
        <Field label="Premio"><Input value={premio} onChange={e => setPremio(e.target.value)} placeholder="Pantalla, cena…" /></Field>
        <Field label="Cierra"><Input type="date" value={cierra} onChange={e => setCierra(e.target.value)} /></Field>
